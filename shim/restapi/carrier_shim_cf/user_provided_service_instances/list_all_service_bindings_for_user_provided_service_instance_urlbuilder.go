...
func (o *ListAllServiceBindingsForUserProvidedServiceInstanceURL) Build() (*url.URL, error) {
	...
	var _path = "/user_provided_service_instances/{guid}/service_bindings"

	guid := o.GUID
	if guid != "" {
		_path = strings.Replace(_path, "{guid}", guid, -1)
	} else {
		return nil, errors.New("guid is required on ListAllServiceBindingsForUserProvidedServiceInstanceURL")
	}
...
func (o *ListAllServiceBindingsForUserProvidedServiceInstanceURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ListAllServiceBindingsForUserProvidedServiceInstanceURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ListAllServiceBindingsForUserProvidedServiceInstanceURL")
	}
