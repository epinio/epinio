...
func (o *CopyAppBitsForAppURL) Build() (*url.URL, error) {
	...
	var _path = "/apps/{guid}/copy_bits"

	guid := o.GUID
	if guid != "" {
		_path = strings.Replace(_path, "{guid}", guid, -1)
	} else {
		return nil, errors.New("guid is required on CopyAppBitsForAppURL")
	}
...
func (o *CopyAppBitsForAppURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on CopyAppBitsForAppURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on CopyAppBitsForAppURL")
	}
