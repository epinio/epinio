...
func (o *GetAppSummaryURL) Build() (*url.URL, error) {
	...
	var _path = "/apps/{guid}/summary"

	guid := o.GUID
	if guid != "" {
		_path = strings.Replace(_path, "{guid}", guid, -1)
	} else {
		return nil, errors.New("guid is required on GetAppSummaryURL")
	}
...
func (o *GetAppSummaryURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetAppSummaryURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetAppSummaryURL")
	}
