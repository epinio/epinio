...
func (o *RetrieveServiceAuthTokenDeprecatedURL) Build() (*url.URL, error) {
	...
	var _path = "/service_auth_tokens/{guid}"

	guid := o.GUID
	if guid != "" {
		_path = strings.Replace(_path, "{guid}", guid, -1)
	} else {
		return nil, errors.New("guid is required on RetrieveServiceAuthTokenDeprecatedURL")
	}
...
func (o *RetrieveServiceAuthTokenDeprecatedURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on RetrieveServiceAuthTokenDeprecatedURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on RetrieveServiceAuthTokenDeprecatedURL")
	}
