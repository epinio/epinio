...
func (o *RetrieveUserURL) Build() (*url.URL, error) {
	...
	var _path = "/users/{guid}"

	guid := o.GUID
	if guid != "" {
		_path = strings.Replace(_path, "{guid}", guid, -1)
	} else {
		return nil, errors.New("guid is required on RetrieveUserURL")
	}
...
func (o *RetrieveUserURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on RetrieveUserURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on RetrieveUserURL")
	}
