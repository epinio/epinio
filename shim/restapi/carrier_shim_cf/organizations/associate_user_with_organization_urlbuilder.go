...
func (o *AssociateUserWithOrganizationURL) Build() (*url.URL, error) {
	...
	var _path = "/organizations/{guid}/users/{user_guid}"

	guid := o.GUID
	if guid != "" {
		_path = strings.Replace(_path, "{guid}", guid, -1)
	} else {
		return nil, errors.New("guid is required on AssociateUserWithOrganizationURL")
	}

	userGUID := o.UserGUID
	if userGUID != "" {
		_path = strings.Replace(_path, "{user_guid}", userGUID, -1)
	} else {
		return nil, errors.New("userGuid is required on AssociateUserWithOrganizationURL")
	}
...
func (o *AssociateUserWithOrganizationURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on AssociateUserWithOrganizationURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on AssociateUserWithOrganizationURL")
	}
