...
func (o *AssociateManagerWithOrganizationURL) Build() (*url.URL, error) {
	...
	var _path = "/organizations/{guid}/managers/{manager_guid}"

	guid := o.GUID
	if guid != "" {
		_path = strings.Replace(_path, "{guid}", guid, -1)
	} else {
		return nil, errors.New("guid is required on AssociateManagerWithOrganizationURL")
	}
	...
	if managerGUID != "" {
		...
		return nil, errors.New("managerGuid is required on AssociateManagerWithOrganizationURL")
	}
...
func (o *AssociateManagerWithOrganizationURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on AssociateManagerWithOrganizationURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on AssociateManagerWithOrganizationURL")
	}
