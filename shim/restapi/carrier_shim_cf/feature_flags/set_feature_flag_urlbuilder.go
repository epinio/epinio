...
func (o *SetFeatureFlagURL) Build() (*url.URL, error) {
	...
	var _path = "/config/feature_flags/{name}"

	name := o.Name
	if name != "" {
		_path = strings.Replace(_path, "{name}", name, -1)
	} else {
		return nil, errors.New("name is required on SetFeatureFlagURL")
	}
...
func (o *SetFeatureFlagURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on SetFeatureFlagURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on SetFeatureFlagURL")
	}
