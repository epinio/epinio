...
func (o *RetrieveServiceBrokerURL) Build() (*url.URL, error) {
	...
	var _path = "/service_brokers/{guid}"

	guid := o.GUID
	if guid != "" {
		_path = strings.Replace(_path, "{guid}", guid, -1)
	} else {
		return nil, errors.New("guid is required on RetrieveServiceBrokerURL")
	}
...
func (o *RetrieveServiceBrokerURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on RetrieveServiceBrokerURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on RetrieveServiceBrokerURL")
	}
