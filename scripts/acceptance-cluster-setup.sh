...
if [ -z ${EXPOSE_ACCEPTANCE_CLUSTER_PORTS+x} ]; then
  ...
  k3d cluster create $CLUSTER_NAME --network $NETWORK_NAME --registry-config $TMP_CONFIG --image "$K3S_IMAGE" --api-port host.k3d.internal:6443 $EPINIO_K3D_INSTALL_ARGS
else
  # Exposing ports on the host:
  k3d cluster create $CLUSTER_NAME --network $NETWORK_NAME --registry-config $TMP_CONFIG -p '80:80@server:0' -p '443:443@server:0' --image "$K3S_IMAGE" --api-port host.k3d.internal:6443 $EPINIO_K3D_INSTALL_ARGS
fi
