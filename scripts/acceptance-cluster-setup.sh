...
if [ -z ${EXPOSE_ACCEPTANCE_CLUSTER_PORTS+x} ]; then
  ...
  k3d cluster create $CLUSTER_NAME --network $NETWORK_NAME --image "$K3S_IMAGE" --api-port 172.19.0.1:6550 --volume /etc/resolv.conf:/etc/resolv.conf@server:0 $EPINIO_K3D_INSTALL_ARGS
else
