...
func ValidateCV(cv models.CVSettings, decl map[string]models.ChartSetting) []error {
...
func Create(ctx context.Context, cluster *kubernetes.Cluster, app models.AppRef, username string, routes []string, chart string, settings models.CVSettings) error {
...
func Settings(app *unstructured.Unstructured) (models.CVSettings, error) {
	settings, _, err := unstructured.NestedStringMap(app.UnstructuredContent(), "spec", "settings")
	if err != nil {
		return models.CVSettings{}, errors.New("chartname should be string")
	}
