...
// AppSourcePatch uploads a source patch tar and triggers a buildpack stage + swapPodImage.
// Used by the watch feature startup mode. Returns the stage response for polling.
func (c *Client) AppSourcePatch(namespace, name string, file FormFile) (*models.StageResponse, error) {
	...
	return DoWithHandlers(c, endpoint, http.MethodPatch, requestHandler, responseHandler)
}
...
func (c *Client) AppSync(namespace, name string, file FormFile, mode, dest, binaryName string) (models.Response, error) {
	response := models.Response{}
