...
var _ = Describe("Client Gitconfigs", func() {
	Describe("Gitconfigs Errors", DescribeGitconfigsErrors)
})

func DescribeGitconfigsErrors() {
	...
	When("a 500 status code and a JSON error was returned", func() {
		...
		DescribeTable("the APIs are returning an error",
			...
			Entry("gitconfig show", func() (any, error) {
				return epinioClient.GitconfigShow("gitconfig")
			}),
			Entry("gitconfig match", func() (any, error) {
				return epinioClient.GitconfigsMatch("gitconfigprefix")
			}),
			Entry("gitconfig list", func() (any, error) {
				return epinioClient.Gitconfigs()
			}),
		)
	})
}
